...
module Magaz
  class Property < ActiveRecord::Base
    ...
    def self.create_combo(name, options, args = {})
      prop = Property.create(name: name, code: args[:code], property_type: PropertyType.list, static: args[:static], variant: args[:variant], property_kind: PropertyKind.find_by_code(args[:kind_code]))
      options.each { |opt| prop.options << PropertyOption.create(name: opt) }
      prop
    end

    def self.create_number(name, args = {})
      Property.create(name: name, code: args[:code], property_type: PropertyType.number, static: args[:static], variant: args[:variant], property_kind: PropertyKind.find_by_code(args[:kind_code]))
    end

    def self.create_float(name, args = {})
      Property.create(name: name, code: args[:code], property_type: PropertyType.float, static: args[:static], variant: args[:variant], property_kind: PropertyKind.find_by_code(args[:kind_code]))
    end

    def self.create_string(name, args = {})
      Property.create(name: name, code: args[:code], property_type: PropertyType.string, static: args[:static], variant: args[:variant], property_kind: PropertyKind.find_by_code(args[:kind_code]))
    end
